...
builder.Services.AddRazorPages(options =>
{
    ...
    options.Conventions.AuthorizeAreaFolder("Admin", "/Categories", Database.AdminPolicy);



    options.Conventions.AuthorizeAreaFolder("Admin", "/Dashboard", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/Sales", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/SportPrograms", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/MealPlan", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/HealthyReicpe", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/Articles", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/Products", Database.ManagerPolicy);
    options.Conventions.AuthorizeAreaFolder("Admin", "/Consultations", Database.ManagerPolicy);
    ...
    options.Conventions.AllowAnonymousToFolder("/Consultations");
...
builder.Services.AddAuthorization(options =>
{
    ...
    options.AddPolicy(Database.ManagerPolicy,
        policy => policy.RequireAssertion(
            context => context.User.HasClaim(Database.Admin, "true") || context.User.HasClaim(Database.Expert, "true")));
...
app.Use(async (context, next) =>
{
    ...
    {
        context.Request.Path = "/Errors/401";
        await next();
    }
    else if (context.Response.StatusCode == 403)

    {
        context.Request.Path = "/Errors/403";
        await next();
