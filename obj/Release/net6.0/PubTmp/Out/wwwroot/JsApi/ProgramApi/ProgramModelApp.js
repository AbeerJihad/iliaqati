...
function RenderCards(Program) {
    ...
    if (Program.isPay == 1) {
        ...
        details = `  <a id="btndetails" href="/SportProgram/ProgramDetails?id=${Program.id}" class="btn bg-white-with-transparent more-details-btn rounded-pill shadow border-0 p-2 px-3 fw-bold" >
                    مزيد من التفاصيل
                </a>`;
        ...
        details = ``;
        pay = `
                     <a id="btnPay"  title="الدفع" onclick="ShowNotPayProgram(${Program.id},this)" class="rounded-pill btn imgBackground add-to-bag-btn border shadow-sm rounded d-flex fw-bold justify-content-center align-items-center align-middle">
                   الدفع
</a>`;
   ...
   <div class=" p-3">
    <div class="card h-100  shadow-sm meal-plan overflow-hidden">
        ...
        <div class="card-body">
            ...
            <p class="card-title text-black text-start"> ${Program.title}
            </p>
...
function ShowNotPayProgram(id) {
    Swal.fire({
        text: "هل تريد الدفع الان ",
        //icon: 'warning  ',
        ...
        if (result.isConfirmed) {
            window.location = `/SportProgram/Index?id=${id}`;
        }
    })
}
