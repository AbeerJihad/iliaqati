namespace liaqati_master.Areas.Admin.Pages.Dashboard
{
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGet()
        {
            ...
            var ISAdmin = User.HasClaim(c => c.Type == Database.Admin && c.Value == "true");
            var userid = User.FindFirstValue(ClaimTypes.NameIdentifier);

            if (!ISAdmin)
            {
                CountersModel.SportProgramCount = await _dbContext.TblSportsProgram.Where(i => i.Services != null && i.Services.UserId == userid).CountAsync();
                CountersModel.MealPlanCount = await _dbContext.TblMealPlans.Where(i => i.Services != null && i.Services.UserId == userid).CountAsync();
                CountersModel.ExersiesCount = await _dbContext.TblExercises.Where(i => i.UserId == userid).CountAsync();
                CountersModel.HealthyCount = await _dbContext.TblHealthyRecipe.Where(i => i.UserId == userid).CountAsync();
                CountersModel.RevenuesTotal = await _dbContext.TblOrder.SumAsync(order => order.TotalPrice);
                CountersModel.ArticlesCount = await _dbContext.TblArticles.Where(i => i.UserId == userid).CountAsync();
                CountersModel.ProductsCount = await _dbContext.TblProducts.Where(i => i.Services != null && i.Services.UserId == userid).CountAsync();
            ...
            //IOrderedEnumerable<string?>? a = (await _dbContext.TblOrder.ToListAsync())
