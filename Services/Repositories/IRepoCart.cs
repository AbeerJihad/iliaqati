namespace liaqati_master.Services.Repositories
{
    public class IRepoCart : IDisposable
    {
        ...
        public async Task<int> AddToCart(string id)
        {
            ...
            if (cartItem == null)
            {
                cartItem = new CartItem
                {
                    ...
                    UnitPrice= (double)_db.TblServices.SingleOrDefault(p => p.Id == id).Price
                };
        ...
        public async Task<int> RemoveAllItem(List<CartItem> list)
        {
            string userId = GetUserId();
            try
            {
                ...
                {
                    var cartItem = _db.TblCartItem
                                  .Where(a => a.UserId == cart.UserId);

                    if (cartItem is null)
                        throw new Exception("Not items in cart");
                    else
                        _db.TblCartItem.Remove(cart);
                    _db.SaveChanges();
                }
