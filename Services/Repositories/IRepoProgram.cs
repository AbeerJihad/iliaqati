namespace liaqati_master.Services.Repositories
{
    public class IRepoProgram
    {
        ...
        public IRepoProgram(LiaqatiDBContext context, IHttpContextAccessor httpContextAccessor, IRepoFavorite IRepoFavorite, IRepoOrder_Details repoOrder)
        {
            _context = context;
        ...
        public async Task<bool> DeleteProgram(string? id)
        {
            ...
            SportsProgram? sport = SportsProgram.FirstOrDefault(p => p.Id==id);
        ...
        public async Task<List<SportsProgram>> GetbyUserId(string id)
        {
            return await _context.TblSportsProgram.Where(p => p.Services.UserId == id).ToListAsync();
        }
        ...
        public async Task<QueryPageResult<SportProgramVM>> SearchSportsProgram(SportProgramQueryParamters sportProgramQueryParams)
        {
            ...
            if (_HttpContextAccessor.HttpContext is not null)
            {
                ...
                else
                {
                    ...
                    foreach (var items in program2)
                    {

                        var item = (await _repoOrder.GetAllAsync()).FirstOrDefault(p => p.ServiceId == items.Id && p.Order.UserId == user.FindFirstValue(ClaimTypes.NameIdentifier));
