namespace liaqati_master.Services.Repositories
{
    public class IRepoUser
    {
        ...
        {
            IQueryable<ExpertsViewModel> Users = (await _userManager.GetUsersForClaimAsync(new System.Security.Claims.Claim(Database.Expert, "true"))).Select(user => new ExpertsViewModel() {Id=user.Id, FullName = string.Concat(user.Fname, " ", user.Lname), Instagram = user.Instagram, WhatsApp = user.WhatsApp, Photo = user.Photo, Specialization = user.Specialization, Twitter = user.Twitter }).AsQueryable(); ;
